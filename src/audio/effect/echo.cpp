...
namespace age::audio::effect
{
    void echo::set_delay(float value)
    {
        if (m_delay != value)
        {
            if (ensure_handle()) AL_CALL(alEffectf(get_handle(), AL_ECHO_DELAY, value));
            m_delay = value;
        }
    }
    ...
    void echo::set_lr_delay(float value)
    {
        if (m_lr_delay != value)
        {
            if (ensure_handle()) AL_CALL(alEffectf(get_handle(), AL_ECHO_LRDELAY, value));
            m_lr_delay = value;
        }
    }
    ...
    void echo::set_feedback(float value)
    {
        if (m_feedback != value)
        {
            if (ensure_handle()) AL_CALL(alEffectf(get_handle(), AL_ECHO_FEEDBACK, value));
            m_feedback = value;
        }
    }
    ...
    void echo::set_damping(float value)
    {
        if (m_damping != value)
        {
            if (ensure_handle()) AL_CALL(alEffectf(get_handle(), AL_ECHO_DAMPING, value));
            m_damping = value;
        }
    }
    ...
    void echo::set_spread(float value)
    {
        if (m_spread != value)
        {
            if (ensure_handle()) AL_CALL(alEffectf(get_handle(), AL_ECHO_SPREAD, value));
            m_spread = value;
        }
    }
